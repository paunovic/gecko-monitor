...
begin
  ...
  if GetLastError <> ERROR_MORE_DATA then Exit;
...
begin
  ...
  // Sanity check
  if (Index < 0) or (Index >= Length(FServices)) then Exit;
...
begin
  if Value then begin
    ...
    if not Winapi.Windows.GetVersionEx(VersionInfo) then Exit;
...
begin
  if not FAllowLocking then Exit;
...
begin
  ...
  if FLock = nil then Exit;
  // Unlock...
  if not UnlockServiceDatabase(FLock) then Exit;
  FLock := nil;
end;
...
begin
  ZeroMemory(@result, SizeOf(TServiceInfo));
  SearchDependants;
  if (Index < 0) or (Index >= Length(FDependents)) then Exit;
...
begin
  if FHandle <> 0 then begin
    if not QueryServiceStatus(FHandle,Status) then Exit;
  end else begin
...
begin
  ...
  try
    if not (saPauseContinue in Accepts) then Exit;
    if not ControlService(FHandle,SERVICE_CONTROL_CONTINUE,Status) then Exit;
...
begin
  ...
  try
    if not (saPauseContinue in Accepts) then Exit;
    if not ControlService(FHandle,SERVICE_CONTROL_PAUSE,Status) then Exit;
...
begin
  ...
  try
    P := nil;
    if not StartService(FHandle,0,P) then Exit;
    if Wait then WaitFor(SERVICE_RUNNING);
  finally
    CleanupHandle;
  end;
...
begin
  ...
  try
    if not (saStop in Accepts) then Exit;
    if not ControlService(FHandle,SERVICE_CONTROL_STOP,Status) then Exit;
  ...
  try
    ...
    try
      if not EnumDependentServices(FHandle,SERVICE_ACTIVE + SERVICE_INACTIVE, Services^,
                          BytesNeeded, BytesNeeded, ServicesReturned) then Exit;
      // Now process it...
      S := Services;
...
begin
  ...
  try
    ...
    try
      // Perform the query...
      if not QueryServiceConfig(FHandle,Buffer,BytesNeeded,BytesNeeded) then Exit;
      // Analyse the query...
  ...
  try
    ...
    try
      ...
      if not ChangeServiceConfig(FHandle,SERVICE_NO_CHANGE,NewStartTypes[Value],SERVICE_NO_CHANGE,
                                 nil,nil,nil,nil,nil,nil,nil) then Exit;
